...
mongoose.connect("mongodb+srv://harshal011k:b4icunokmidown@cluster0.ls2zwnz.mongodb.net/blogDB")
...
let Us=mongoose.model("Us",usSchema);
let aboutContent=new Us({
...
let homeStartingContent=new Us({
...
app.get("/",(req,res)=>{
  Blog.find({})
  .then((foundItem)=>{
      res.render("home",{para1:homeStartingContent,item:foundItem});
      res.redirect("/");
  })
  .catch((err)=>{
    console.log(err);
...
app.get("/posts/:postId",(req,res)=>{
   ...
   .then((post)=>{
    res.render("post",{title:post,data:post});
   })
   .catch((err)=>{
    console.log(err);
   });
